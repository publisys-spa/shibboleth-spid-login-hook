...
<%
        ...
        boolean shibbolethEnabled = GetterUtil.getBoolean(PropsUtil
                        .get(SHIBBOLETH_ENABLED));
%>
        ...
        <%
                String signedInAs = HtmlUtil.escape(user.getFullName());
                                ...
                                if (themeDisplay.isShowMyAccountIcon()) {
                                        signedInAs = "<a href=\""
                                                        + HtmlUtil.escape(themeDisplay
                                                                        .getURLMyAccount().toString()) + "\">"
                                                        + signedInAs + "</a>";
                                }
        %>

        <%=LanguageUtil.format(pageContext,
							"you-are-signed-in-as-x", signedInAs)%>
    </c:when>
    <c:when test="<%=shibbolethEnabled%>">
        ...
        <c:if
            test='<%=SessionMessages.contains(request,
								"shibbError")%>'>
            <h5 class="portlet-msg-error">
                <%=(String) SessionMessages.get(request,
								"shibbError")%>
            </h5>
        </c:if>

        <%
                String _href = "/c/portal/login" + "?ltype=shibb";
        %>
        <aui:a href="<%=_href%>" label="sign-in" />
    ...
    <c:otherwise>
        ...
        <aui:form action="<%=loginURL%>" method="post" name="fm">
            ...
            <c:if
                ...
                <%
                        String userEmailAddress = (String) SessionMessages
                                                                        .get(request, "user_added");
                                                        String userPassword = (String) SessionMessages.get(
                ...
                <div class="portlet-msg-success">
                    <c:choose>
                        <c:when
                            ...
                            <%=LanguageUtil
                                                            .format(pageContext,
                                                                            "thank-you-for-creating-an-account-your-password-has-been-sent-to-x",
                                                                            userEmailAddress)%>
                        </c:when>
                        <c:otherwise>
                            <%=LanguageUtil
            ...
            <aui:fieldset>
                ...
                <aui:input label="<%=loginLabel%>" name="login" type="text"
                           value="<%=login%>" />
                ...
                <c:if
                    ...
                    <aui:input checked="<%=rememberMe%>" inlineLabel="left"
